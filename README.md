...
the ***mqtt1Topic1***: Topic is used by the device as publish channel
 - ***MQTT Subscribe t and the ***mqtt1Topic2***: Topic used by the device is used as subscribed channel 
...
The module is publishing the output state via the json string formatted above on the ***MQTT Publish tmqtt1Topic***1 every time any state of the 4 output is modified either by the web interface or via the MQTT subscribed channel.

It publishes the output state during startup, when the http default page have been open and on 1 minute regular bases.

The module listen to the ***MQTT Subscribe t mqtt1Topic***2 and is expecting the Json string as formatted as described above. It changes the output state according to the Json string.
...
All the parameters are store in the file config.json store in the data partition.
    ...
    "mqtt1ClientId":"<YOUR  MQT  TMQTT CLIENT  ID>",
...
4 6. (maybe) implementing HTTPS connection for the internal webserver.
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbMTQ0ODQyOTEzOF19
-->